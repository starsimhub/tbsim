...
def _build_sim(
    *,
    ...
    connectors = ResistanceConnector() if with_resistance else None
    sim = tbsim.Sim(
        tb_model=tb,
        sim_pars=dict(
...
def _cum_new_inh_r(sim):
    ...
    analyzer = next(
        (
            a for a in sim.analyzers.values()
            if isinstance(a, StrainResults) or a.__class__.__name__ == StrainResults.__name__
        ),
        None,
    )
    if analyzer is None:
