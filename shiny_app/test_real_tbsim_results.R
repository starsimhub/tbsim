# Test Real TBsim Results Integration
# This script tests that we can extract results from sim.results object
...
cat("Testing real TBsim results extraction...\n")
...
tryCatch({
  ...
  if (length(available_fields) > 0) {
    ...
    cat("\n🎉 Real TBsim results extraction test passed!\n")
  ...
  cat("❌ Error:", e$message, "\n")
  cat("Real TBsim results extraction failed.\n")
