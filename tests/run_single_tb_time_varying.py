...
class time_varying_presym_active(ss.Intervention):
    def __init__(self, pars=None, *args, **kwargs):
        super().__init__()
        self.define_pars(
            tb_parameter = 'rate_presym_to_active',
            #baseline = 
            ti = [sc.date('1995-01-01'),sc.date('2014-01-01')],
            rc = [1, 0.5],
        )
    ...
    def step(self):
        ...
        # interpolate the values
        rc = np.interp(self.t.ti, t_index, self.pars.rc)
        self.sim.diseases.tb.pars[self.pars.tb_parameter] *= rc
        return
...
def make_tb():
    ...
    pop = ss.People(n_agents=np.round(np.random.normal(loc=10_000, scale=50)))
    demog = [
        #ss.Pregnancy(pars=dict(fertility_rate=65)),
        ss.Births(pars=dict(birth_rate=108.65)), #30
        ss.Deaths(pars=dict(death_rate=98)) 
    ]
    ...
    tb_pars = dict(
        ...
        rate_active_to_clear=ss.perday(8.5e-4),#ss.perday(2.4e-4),
    ...
    decrease_presym_to_active = time_varying_presym_active(
        dict(ti = [sc.date('1995-01-01'),sc.date('2014-01-01')],
             rc = [1, 1]
             ))
    ...
    sim_pars = dict(
        ...
        unit='day', dt=60,
        start=ss.date('1850-01-01'), stop=ss.date('1989-12-31') 
        )
    ...
    sim = ss.Sim(
        ...
        interventions=[decrease_beta, decrease_presym_to_active],
    ...
    ms = ss.MultiSim(sim, iterpars=dict(rand_seed=np.random.randint(0, 1e6, n_reps)), 
                     initialize=True, debug = False, parallel=True)
