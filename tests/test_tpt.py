"""Tests for TPTProduct, TPTRoutine, and TPTHousehold (tbsim/interventions/tpt.py)."""
...
def make_sim(*interventions, n=200, init_prev=0.40,
    ...
    return ss.Sim(
        ...
        pars=dict(dt=ss.days(7), start=start, stop=stop, rand_seed=0, verbose=0),
    )


def quick_regimen(p_complete=1.0, modifier=0.5, dur_protection=ss.years(10),
                  dur_treatment=ss.days(0)):
    return TPTRegimen(
        name='QUICK',
        category=RegimenCategory.RIFAMYCIN_SHORT,
        dur_treatment=ss.constant(v=dur_treatment),
...
def make_dhs_data(n_hh=60):
    age_strings = [
        sc.strjoin(np.random.randint(1, 70, np.random.randint(2, 6)).tolist())
        for _ in range(n_hh)
    ]
    return sc.dataframe(hh_id=np.arange(n_hh), ages=age_strings)


def make_hh_sim(*interventions, init_prev=0.2, n_hh=60):
    return ss.Sim(
        diseases=tbsim.TB_LSHTM(name='tb', pars={'init_prev': init_prev}),
        networks=sse.HouseholdDHSNet(dhs_data=make_dhs_data(n_hh)),
        interventions=list(interventions),
        pars=dict(dt=ss.days(7), start=ss.date('2000-01-01'),
                  stop=ss.date('2010-12-31'), rand_seed=0, verbose=0),
    )


# TPTRoutine — configuration

def test_default_coverage_and_eligible_states():
    sim = make_sim(TPTRoutine())
    sim.init()
    tpt = sim.interventions['tptroutine']
    assert '0.5' in str(tpt.pars.coverage)
    assert tpt.pars.eligible_states == [TBSL.INFECTION]


def test_custom_regimen_parameters_accessible():
    reg = TPTRegimen(
        name='CUSTOM', category=RegimenCategory.ISONIAZID_LONG,
    ...
    itv = TPTRoutine(product=TPTProduct(regimen=reg), pars={
    ...
    assert hasattr(tpt.product.regimen, 'dur_treatment')
    assert hasattr(tpt.product.regimen, 'dur_protection')


# TPTProduct — eligibility filters
...
# TPTProduct — two-phase model and protection mechanics
...
def test_protection_expiry():
    sim = inited_sim(regimen=quick_regimen(dur_treatment=ss.days(1),
                                           dur_protection=ss.days(1)))
    tpt = sim.interventions['tptroutine']
    for _ in range(5):
        tpt.step()
...
def test_expiry_clears_offered_flag():
    itv = TPTRoutine(product=quick_regimen(dur_protection=ss.days(1)),
                     pars={'coverage': ss.bernoulli(p=1.0)})
...
# TPTRoutine — delivery logic

def test_routine_offered_gate():
    sim = inited_sim(coverage=1.0)
    itv = sim.interventions['tptroutine']
    itv.step()
...
def test_age_range_filter():
    itv = TPTRoutine(pars={'coverage': ss.bernoulli(p=1.0), 'age_range': [15, 50]})
    sim = make_sim(itv)
    sim.init()
    sim.interventions['tptroutine'].step()
    initiated = sim.interventions['tptroutine'].tpt_initiated.uids
    if len(initiated):
        ages = np.asarray(sim.people.age[initiated])
        assert np.all(ages >= 15) and np.all(ages <= 50)


def test_result_keys_present():
    sim = make_sim(TPTRoutine())
    sim.run()
    res = sim.interventions['tptroutine'].results
    assert 'n_initiated' in res and 'n_protected' in res
...
def test_hh_init():
    ...
    assert itv._hh_index is not None
...
def test_hh_full_run():
    sim = make_hh_sim(TPTHousehold(pars={'hh_coverage': ss.bernoulli(p=0.8)}),
                      init_prev=0.3)
    sim.run()
    res = sim.interventions['tpthousehold'].results
    assert 'n_initiated' in res and 'n_protected' in res
