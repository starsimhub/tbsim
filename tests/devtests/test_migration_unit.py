...
def test_age_dict_bins_constructed_correctly():
    ...
    sim.init()
    mig = _migration(sim)
    assert np.array_equal(mig.age_lows, np.array([0.0, 20.0, 50.0]))
    assert np.array_equal(mig.age_highs, np.array([20.0, 50.0, 80.0]))
...
def test_invalid_immigration_age_bins_fall_back_without_breaking_tb_init():
    ...
    assert int(TBS.SUSCEPTIBLE) in mig.dist_tb_state.pars.a
...
def test_emig_weights_match_loop_reference():
    ...
    for lo, hi, w in zip(mig.emig_age_lows, mig.emig_age_highs, mig.emig_age_weights):
...
def test_emig_age_distribution_warns_on_empty():
    with pytest.warns(UserWarning):
        ...
        sim.init()
    assert _migration(sim).emig_age_lows is None
...
def test_adjust_arrivals_compensates_excess_emigration():
    ...
    sim.init()
    mig = _migration(sim)
    baseline = mig.baseline_population
...
def test_emigration_age_bins_respect_max_age_upper_bound():
    ...
    sim.init()
    mig = sim.demographics.migration
    assert mig.emig_age_lows is not None
    assert np.allclose(mig.emig_age_lows, [0, 20])
    assert np.allclose(mig.emig_age_highs, [20, 40])
