...
def test_tb_infectious():
    ...
    assert tb.infectious.all()
    tb.state[:] = tbsim.TBS.ACTIVE_SMNEG
    assert tb.infectious.all()
    tb.state[:] = tbsim.TBS.ACTIVE_EXPTB
    assert tb.infectious.all()
    tb.state[:] = tbsim.TBS.NONE
...
def test_update_pre():
    ...
    assert len(tb.state[tb.state == tbsim.TBS.ACTIVE_SMNEG]) > 0
    
    print("none", tb.state[tb.state == tbsim.TBS.NONE])
    print("Slow:", tb.state[tb.state == tbsim.TBS.LATENT_SLOW])
    print("Fast:", tb.state[tb.state == tbsim.TBS.LATENT_FAST])
    print("Active Presymp:", tb.state[tb.state == tbsim.TBS.ACTIVE_PRESYMP])
    print("Active ExpTB:", tb.state[tb.state == tbsim.TBS.ACTIVE_EXPTB])
    print("Active Smear Negative: ", tb.state[tb.state == tbsim.TBS.ACTIVE_SMNEG])
    print("Active Smear Positive: ", tb.state[tb.state == tbsim.TBS.ACTIVE_SMPOS])
...
def test_start_treatment_no_active_tb(tb):
    uids = ss.uids([1, 2, 3])
    tb.state[uids] = tbsim.TBS.LATENT_SLOW  # No active TB
...
def test_start_treatment_with_active_tb(tb):
    uids = ss.uids([1, 2, 3])
    tb.state[uids] = tbsim.TBS.ACTIVE_SMPOS  # Active TB
    num_treated = tb.start_treatment(uids)
...
def test_start_treatment_mixed_states(tb):
    ...
    tb.state[uids_active] = tbsim.TBS.ACTIVE_SMPOS  # Active TB
    tb.state[uids_non_active] = tbsim.TBS.LATENT_SLOW  # No active TB
...
def test_start_treatment_exptb(tb):
    uids = ss.uids([1, 2, 3])
    tb.state[uids] = tbsim.TBS.ACTIVE_EXPTB  # Active extrapulmonary TB
    num_treated = tb.start_treatment(uids)
    assert num_treated == len(uids)
    assert tb.on_treatment[uids].all()
    assert (tb.rr_death[uids] == 0).all()
...
def test_set_prognoses_susceptible_to_infected(tb):
    ...
    fast_uids = uids[tb.state[uids] == tbsim.TBS.LATENT_FAST]
    assert not tb.susceptible[fast_uids].any()

    # Slow progressors should still be susceptible
    slow_uids = uids[tb.state[uids] == tbsim.TBS.LATENT_SLOW]
...
def test_set_prognoses_active_tb_state(tb):
    uids = ss.uids([1, 2, 3])
    tb.pars.active_state.rvs = lambda uids: np.array([tbsim.TBS.ACTIVE_SMPOS, tbsim.TBS.ACTIVE_SMNEG, tbsim.TBS.ACTIVE_EXPTB])
    
    tb.set_prognoses(uids)
    
    assert np.array_equal(tb.active_tb_state[uids], np.array([tbsim.TBS.ACTIVE_SMPOS, tbsim.TBS.ACTIVE_SMNEG, tbsim.TBS.ACTIVE_EXPTB]))
...
def test_p_latent_to_presym():
    ...
    tb.state[:] = tbsim.TBS.LATENT_SLOW  # Assuming all agents are in the latent slow TB state
...
def test_p_presym_to_active():
    ...
    # Temporarily unlock attributes for testing
    tb._lock_attrs = False
    tb.state[:] = tbsim.TBS.ACTIVE_PRESYMP  # Assuming all agents are in the pre-symptomatic active TB state
    ...
    assert (tb.state[presym_uids] == tbsim.TBS.ACTIVE_PRESYMP).all(), "All tested UIDs should be in the pre-symptomatic state"
...
def test_p_active_to_death( ):
    ...
    expected_rate[tb.state[active_uids] == tbsim.TBS.ACTIVE_SMPOS] = tb.pars.rate_smpos_to_dead.rate
    expected_rate[tb.state[active_uids] == tbsim.TBS.ACTIVE_SMNEG] = tb.pars.rate_smneg_to_dead.rate
    ...
    assert (tb.state[active_uids] == tbsim.TBS.ACTIVE_SMPOS).any() or (tb.state[active_uids] == tbsim.TBS.ACTIVE_SMNEG).any() or (tb.state[active_uids] == tbsim.TBS.ACTIVE_EXPTB).any(), "Ensure at least some active TB states are set for testing"
...
def test_latent_to_active_presymptomatic_transition():
    ...
    transitioned = tb.state[latent_uids] == tbsim.TBS.ACTIVE_PRESYMP
    print(transitioned)
...
def test_presymptomatic_to_active_transition():
    ...
    transitioned = (
        (tb.state[presym_uids] == tbsim.TBS.ACTIVE_SMPOS)
        | (tb.state[presym_uids] == tbsim.TBS.ACTIVE_SMNEG)
        | (tb.state[presym_uids] == tbsim.TBS.ACTIVE_EXPTB)
    )
...
def test_active_to_cleared_transition():
    ...
    cleared = tb.state[active_uids] == tbsim.TBS.NONE
...
def test_active_to_death_transition():
    ...
    died = tb.state[active_uids] == tbsim.TBS.DEAD
    # total = len(died[died])
