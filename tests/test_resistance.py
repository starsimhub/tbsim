"""
Tests for the drug-resistance overlay (StrainSpec, StrainRegistry, StrainProfile,
...
class TestStrainRegistry:
    def test_basic_registry(self):
        reg = StrainRegistry(make_default_strains())
        assert reg.n == 3
    ...
    def test_explicit_drug_ordering(self):
        reg = StrainRegistry(make_default_strains(), drugs=['BDQ', 'RIF', 'INH'])
...
class TestTBStrainHook:
    def test_no_strain_profile_when_not_configured(self):
        tb = tbsim.TB()
        assert not hasattr(tb, 'strain_profile')

    def test_strain_profile_created_when_configured(self):
        tb = tbsim.MultiStrainTB(strains=make_default_strains())
        assert isinstance(tb.strain_profile, StrainProfile)
        assert tb.strain_profile.registry.n == 3

    def test_strain_states_registered_on_tb(self):
        tb = tbsim.MultiStrainTB(strains=make_default_strains())
        for name in tb.strain_profile.names:
...
# ---------------------------------------------------------------------------
# StrainProfile per-agent operations
# ---------------------------------------------------------------------------
...
class TestStrainProfile:
    def test_initial_seeding_assigns_strain(self):
        ...
        pan_carriers = tb.strain_profile.carriers('pan')
        rif_carriers = tb.strain_profile.carriers('rif_r')
    ...
    def test_clear_all_removes_all_strains(self):
        ...
        tb = tbsim.get_tb(sim)
        infected = tb.infected.uids
        tb.strain_profile.clear_all(infected)
        for uid in tb.strain_profile.registry.uids:
            assert not tb.strain_profile.carries(uid, infected).any()
    ...
    def _clean_targets(self, tb, count):
        ...
        counts = tb.strain_profile.n_strains_per_agent(all_uids)
    ...
    def test_add_strain_blocks_duplicate(self):
        ...
        tb.strain_profile.add_strain(target, 'pan')
        # Second add should be a no-op (returns no new uids)
        added_again = tb.strain_profile.add_strain(target, 'pan')
        assert len(added_again) == 0
        assert tb.strain_profile.carries('pan', target).all()
    ...
    def test_replace_strain(self):
        ...
        tb.strain_profile.add_strain(target, 'pan')
        tb.strain_profile.replace_strain(target, old='pan', new='rif_r')
        assert not tb.strain_profile.carries('pan', target).any()
        assert tb.strain_profile.carries('rif_r', target).all()
    ...
    def test_effective_rel_trans_takes_max_fitness(self):
        ...
        tb = tbsim.get_tb(sim)
        target = self._clean_targets(tb, 2)
        tb.strain_profile.add_strain(target, 'pan')         # fitness 1.0
        tb.strain_profile.add_strain(target, 'rif_r')       # fitness 0.9
        eff = tb.strain_profile.effective_rel_trans(target)
    ...
    def test_n_strains_per_agent(self):
        ...
        tb = tbsim.get_tb(sim)
        target = self._clean_targets(tb, 3)
        tb.strain_profile.add_strain(target[:2], 'pan')
        tb.strain_profile.add_strain(target[1:], 'rif_r')
        counts = tb.strain_profile.n_strains_per_agent(target)
...
class TestResistanceSim:
    def test_short_sim_runs_with_overlay(self):
        ...
        sim.run()
        tb = tbsim.get_tb(sim)
        assert tb.strain_profile is not None
        # At least some agents should carry a strain at some point
        any_strain = False
        for uid in tb.strain_profile.registry.uids:
            if len(tb.strain_profile.carriers(uid)) > 0:
                any_strain = True
                break
    ...
    def test_dead_agents_carry_no_strain(self):
        ...
        counts = tb.strain_profile.n_strains_per_agent(dead)
    ...
    def test_cleared_agents_carry_no_strain(self):
        ...
        counts = tb.strain_profile.n_strains_per_agent(cleared)
    ...
    def test_susceptible_agents_carry_no_strain(self):
        ...
        counts = tb.strain_profile.n_strains_per_agent(susc)
        assert (counts == 0).all()
    ...
    def test_no_duplicate_strains_per_agent_no_superinfection(self):
        ...
        all_uids = ss.uids(np.arange(len(tb.sim.people)))
        counts = tb.strain_profile.n_strains_per_agent(all_uids)
        assert counts.max() <= tb.strain_profile.registry.n
