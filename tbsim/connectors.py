...
from tbsim import TB, Malnutrition, HIV, HIVStage
...
class TB_HIV_Connector(ss.Connector):
    ...
    This connector uses the HIV stage from the HIV disease model (with stages ACUTE, LATENT, AIDS)
    ...
    def compute_tb_activation_rr(tb, hiv, uids, base_factor=1.0):
        """
        Compute TB activation relative risk multiplier based on HIV stage.
        ...
        stages = hiv.stage[uids]
        for i, s in enumerate(stages):
            if s == HIVStage.ACUTE:
                rr[i] = 1.5
            elif s == HIVStage.LATENT:
                rr[i] = 2.0
            elif s == HIVStage.AIDS:
                rr[i] = 3.0
            else:
                rr[i] = 1.0
    ...
    def compute_tb_rel_sus(tb, hiv, uids, baseline=1.0):
        """
        Compute TB relative susceptibility multiplier based on HIV stage.
        
        Example multipliers:
          - HIV ACUTE: baseline * 1.5
