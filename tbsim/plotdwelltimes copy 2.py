...
# TODO: VERIFY - clean up / finish.
def plot_p3(file_path):
...
# TODO: VERIFY - clean up / finish.
def plot_stacked_bars_states_per_agent(file_path):
    import plotly.express as px
    # Load the CSV file
    df = pd.read_csv(file_path)
...
# TODO: VERIFY - clean up / finish.
def plot_stacked_bars_states_per_agent_clean(file_path):
    import plotly.express as px
    # Load the CSV file
    df = pd.read_csv(file_path)
...
if __name__ == "__main__":
    ...
    dwell_time_logger = pd.read_csv(file_path, na_values=[], keep_default_na=False)
    ...
    # ##parallel_coordinates(dwell_time_logger=dwell_time_logger)
    # ##parallel_categories(dwell_time_logger=dwell_time_logger)
...
# TODO: clean up:
# def parallel_coordinates(file_path=None, dwell_time_logger=None):
#     import pandas as pd
#     import plotly.express as px
...
# TODO: clean up
# def parallel_categories(file_path=None, dwell_time_logger=None):
#     import pandas as pd
#     import plotly.express as px
...
#     # Create bins for dwell times
#     bins = np.linspace(0, 60 * 12, 13)
#     bin_labels = [f"{int(b)}-{int(b + 60)} days" for b in bins[:-1]]
#     df['dwell_time_bin'] = pd.cut(df['dwell_time'], bins=bins, labels=bin_labels, include_lowest=True)

#     fig = px.parallel_categories(df, dimensions=['agent_id', 'state_name', 'going_to_state', 'dwell_time_bin'],
