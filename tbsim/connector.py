...
class TB_Nutrition_Connector(ss.Connector):
    ...
    def update_rel_prog(self, tb, change_uids, k_old, k_new, state, rate, ti, dt):
        ...
        if state != TBS.LATENT_SLOW and state != TBS.LATENT_FAST:
            raise ValueError(f'State passed to update_rel_prog must be LATENT_SLOW or LATENT_FAST, not {state}')
        ...
        if state==TBS.LATENT_SLOW: 
            tb.rel_LS_prog[change_uids] = k_new # Update slow rel_prog
        else:
            tb.rel_LF_prog[change_uids] = k_new # Update fast rel_prog
