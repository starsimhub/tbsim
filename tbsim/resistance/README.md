# `tbsim.resistance` — multi-strain / drug-resistance extension

This package turns TBsim's single-strain TB model into a model of **circulating drug-resistant strains**. This README explains **how** the extension is implemented, feature by feature, for readers who want to understand or extend the code — it names the concrete classes, methods, parameters, and results, and gives the design rationale. For a hands-on, *usage*-focused walkthrough see the [drug resistance tutorial](../../docs/tutorials/resistance_tutorial.qmd).

## 1. Overview

The extension is built on one architectural idea: rather than write a bespoke multi-strain transmission engine, it **reuses Starsim's common-random-number force-of-infection (FOI) engine** and layers a strain overlay on top of the existing agent-level TB state machine. Concretely:

- A **strain** is an `n`-bit resistance profile over an ordered drug set; for `n` drugs there are `m = 2ⁿ` strains.
...
The deterministic **two-strain ODE** (`tbsim.compartmental.TwoStrainODE`, a port of `two_strain_ode.R`) is the validation reference; §12 shows the agent-based model reproducing it.
...
`TBResistant(TB)` inherits the entire single-strain natural-history state machine (`TBS`: SUSCEPTIBLE → INFECTION → NON_INFECTIOUS → ASYMPTOMATIC → SYMPTOMATIC → TREATMENT/CLEARED/DEAD) and adds the strain overlay. The natural history stays **agent-level** — one `TB.state` per agent — and progression rates are strain-count-independent; strains ride on top.

`TBResistant.__init__` defaults the module `name` to `'tb'` so the standard TBsim interventions (which key on disease `'tb'`) integrate unchanged, and it resolves the reinfection-coupling defaults (`rr_reinfection_inf` ← `rr_reinfection_rec`, `rr_reinfection_non` ← `rr_reinfection_inf`). `TBResistant.agnostic()` is a convenience factory that builds a single-drug, no-resistance instance behaving like plain `tbsim.TB`.
...
This is the reuse-the-FOI design in full.

**`step_bookkeeping` sets three arrays:**

- `rel_trans = Strains.max_fitness(strain_mask)` — an infectious agent transmits at its **fittest** carried strain's rate. Superinfection therefore does not lower total infectiousness (relative effective contact rate = max fitness). Asymptomatic agents are additionally scaled by `trans_asymp`.
...
- **identical-strain superinfection** — if the target already carries the drawn strain, its per-strain **count** is incremented; the infection clock resets and the event is counted in `new_identical_superinf`.
...
Two constraints hold by construction: infectees acquire only strains the source carries, and no resistance emerges during transmission. A source carrying `{RIF}` and `{RIF,BDQ}` transmits at rate `r_RIF·β`, split ∝ fitness across those strains.
...
- **Progression bottleneck** (`_bottleneck`): only at `→ ASYMPTOMATIC`, a multi-strain agent keeps all strains with probability `p_multi` (default 1); otherwise exactly one strain progresses, chosen with equal probability by default (`prog_select='random'`) or fitness-weighted (`prog_select='fitness'`). `INFECTION → NON_INFECTIOUS` retains all strains.
- **Clearance clears everything**: natural clearance/resolution (`INFECTION → CLEARED`, `NON_INFECTIOUS → CLEARED`) sets `strain_mask = 0` — immune-mediated clearance is strain-agnostic, in contrast to treatment/TPT which clear selectively.
- **Superinfection rate modifiers**: `rr_prog_super` (ψ) scales the progression rates of multi-strain agents; `rr_clear_super` (ω) scales their natural-clearance rate. Both default to 1.

## 7. De-novo resistance acquisition (`_denovo`)

At progression out of `INFECTION` (to NON_INFECTIOUS or ASYMPTOMATIC), each carried strain independently acquires resistance to each not-yet-resistant drug with probability `p_rand[drug]` (`p_rand` is a per-drug dict, so RIF's rate can be 0 while another drug's is positive). The drug bits a source strain acquires this step combine into one resistant target strain, which is either **added** (`prog_resist_mode='mixed'` → superinfection, the default) or **swapped in** (`'replacement'`). Each drug has its own CRN stream so per-drug draws are independent.

Because the strain space is the full `2ⁿ`, the resistant target strain **always exists** — de-novo events are never silently dropped (a genuine advantage of the bitmask over an explicitly-enumerated strain list). Events feed the `new_denovo_resistance` result.

## 8. Treatment (`TxR` + `TxDeliveryR`)

Treatment follows TBsim's product/delivery split.

**`TxR`** (product) holds the regimen's per-strain efficacy and acquisition rules:

- `eff_by_id[j]` = `base_efficacy` × ∏ `resist_penalty[d]` over the *regimen* drugs `d` that strain `j` resists. Resistance to a drug **outside** the regimen does not reduce efficacy. An explicit `efficacy_by_strain` vector bypasses this derivation and sets `eff_by_id` verbatim.
- `adherence` gates the whole course through one per-agent completion draw, inducing an agent-level correlation across strains (a non-completer clears nothing). It is either a float (one regimen-level probability shared by all agents) or a callable `uids -> per-agent probability` (a regimen-level distribution that varies by agent).
...
- `acquire(uids, surv, states)` applies acquisition-on-failure as **replacement**: once per agent per regimen drug, a surviving strain susceptible to that drug becomes resistant, with probability `q_acq[drug]` scaled by a per-TB-state RR (`acq_state_rr`, default 1 for ASYMPTOMATIC/SYMPTOMATIC and 0 elsewhere).

Together `roll_survivors` + `acquire` reproduce the ODE's treatment-outcome operator `π(m→s)` **exactly**, including the AB→B collapse when a failed superinfection acquires resistance; `tests/test_resistance.py::test_treatment_outcome_operator_matches_ode` checks this against the ODE `pi_*` formulas.

**`TxDeliveryR`** (delivery) initiates treatment from the active states at state-specific rates (`rate_asym`, `rate_sym`) or from a custom `eligibility` callable, freezes the per-strain outcome at initiation, and resolves it after `dur_treatment`: fully cleared → CLEARED (with post-treatment reinfection protection), otherwise back to the state treated from, carrying the surviving (and possibly newly resistant) strains. It stamps a durable, cross-regimen `tb.ti_last_treatment`, so `failure_case_eligibility(within=...)` can classify retreatment vs new cases, and `retreat_after` imposes a per-agent refractory period. Results: `n_treated`, `n_success`, `n_failure`, `n_acquired`.

### Treatment monitoring and mid-course switching

Prematurely stopping/changing an ongoing regimen is realized by two composable pieces:

- `treatment_monitoring_eligibility(tx_name, after_steps, every_steps, require=)` returns a `sim → uids` callable selecting agents who have been on a named course past a time-under-treatment threshold (tracked via `ti_treatment_start`), optionally narrowed by another eligibility callable.
...
The composable eligibility helpers `eligibility_all` / `eligibility_any` (intersection / union of `sim → uids` callables) and `will_fail(tx_name)` (agents whose pre-rolled course outcome is a failure) support richer routing.

## 9. Diagnostics (`DST` + `DSTDelivery`)

`DST` produces an **observed** `n`-bit phenotype from an agent's carried strains. It applies sensitivity/specificity **at the strain level** and aggregates:
...
Independent per-strain draws are essential: they make a phenotype carried by *several* strains **more** likely to be detected (`P = 1 − (1−sens)ᵏ`), and make `p_strain_obs < 1` genuinely reduce sensitivity via strain drop-out.

`DSTDelivery` stores the observed profile (`dst_profile`) and exposes routing helpers: `observed_resistant(drug)` (single drug) and `matches(RIF=True, BDQ=False, ..., max_age=)` (composable multi-drug, optionally within a result-age window), each returning `sim → uids` eligibility callables that resolve the sim's own DST instance at call time (copy-safe). A stored result older than `result_validity` is wiped so the agent must be re-tested. Feed these to `TxDeliveryR(eligibility=..., supersedes=[...])` to route or switch regimens by observed phenotype.

## 10. Preventive therapy (`TPTRx`)

`TPTRx` subclasses the existing `tbsim.TPTTx` and makes sterilization **per strain**: among agents in the sterilize branch it clears only strains susceptible to *every* regimen drug (`_covered_mask`), so a resistant strain in a co-infected latent agent **survives** and can later progress and transmit; the agent reaches CLEARED only once no strain remains. This is the Mills–Cohen "preventive therapy unmasks resistance" dynamic — a less-fit resistant strain that would be out-competed on its own gains share when INH clears the susceptible strain it competes with.

TPT-driven resistance **acquisition** is applied to the "TPT was ineffective" cohort via `_apply_neither_branch`: a susceptible carried strain mutates to its resistant counterpart at per-drug rate `p_tpt_acq[drug]`, scaled by a per-TB-state RR whose defaults follow a gradient (INFECTION 0.05, NON_INFECTIOUS 0.5, ASYMPTOMATIC/SYMPTOMATIC 1.0). Use `TPTRx` inside any TPT delivery, e.g. `tbsim.TPTSimple(product=tbsim.TPTRx(...))`, with `p_sterilize > 0`.

## 11. Results and analyzers

`TBResistant` records, alongside the standard TB results: `frac_resist` (resistant fraction of active TB), `frac_super` (superinfected fraction), `frac_resist_<drug>` (per drug), and the per-step flux counters `new_identical_superinf`, `new_denovo_resistance`, `new_transmitted_resistance`.

Two analyzers add cross-cutting views:

- **`ResistanceStats`** decomposes new resistance into **de-novo**, **treatment-acquired**, **transmitted**, and **TPT-acquired** flux (`flux_denovo` / `flux_txacq` / `flux_transmitted` / `flux_tptacq`) and exposes `to_df(sim)`. In a treated epidemic, transmission dominates once resistance is established, seeded and topped up by the other channels.
...
The agent-based model is validated against `tbsim.compartmental.TwoStrainODE` (a port of `two_strain_ode.R`, verified bit-identical to the R reference). The two share the LSHTM natural-history rates exactly; seeded identically over a shared window, the ABM tracks the deterministic ODE across the single-strain reduction, competitive exclusion, and treatment-driven selection.

The CI-level checks live in the main test suite:

- `tests/test_resistance.py` — the single-strain reduction, strain symmetry, population conservation, and the treatment-outcome operator, plus the behavioral levers (competitive exclusion, treatment selecting for resistance, superinfection requiring σ > 0, de-novo mixed-vs-replacement, the DST/TPT mechanics, and the `ResistanceStats` origin decomposition).
- `tests/test_compartmental.py::test_two_strain_abm_vs_ode` — a matched-IC ABM↔ODE comparison across the single-strain, competitive-exclusion, and treatment-selection scenarios.
...
`TxR`: `base_efficacy`, `resist_penalty` (per-drug dict), `efficacy_by_strain`, `adherence`, `q_acq` (per-drug dict), `acq_state_rr`, `regimen_drugs`. `TxDeliveryR`: `rate_asym`, `rate_sym`, `dur_treatment`, `eligibility`, `supersedes`, `retreat_after`, `treat_latent`. `DST`: `sens`, `spec` (scalar or per-drug dict), `p_strain_obs`; `DSTDelivery`: `result_validity`. `TPTRx`: `regimen_drugs`, `p_tpt_acq`, `acq_state_rr` (+ the base `TPTTx` `efficacy`/`p_sterilize`/durations).

## 14. Mapping to the reference ODE's questions of interest

Each "question of interest" from the reference two-strain ODE is a knob on this one model:
...
**Q5 is the one deliberate gap.** The bitmask + reuse-FOI design implements only the transmission *bottleneck* (a superinfected source transmits at its fittest strain's rate, then one strain is drawn ∝ `count × fitness`). Fully independent per-strain transmission would require per-strain FOI channels, which the shared-engine design does not provide; it remains available in the ODE (`transmission_independent`) as the reference for that question.

## 15. Design decisions and known limitations

- **Bitmask vs. declared strains.** The `2ⁿ` bitmask matches the reference model exactly and guarantees that de-novo/acquired resistance always has a target strain (no silently-dropped emergence). The cost is that the strain-membership decode is `O(m)`; this is negligible for realistic drug counts (`n ≈ 2–4` → 4–16 strains) but would grow for many independent drugs.
- **Adherence** supports both a single regimen-level probability (float) and a per-agent distribution (callable) applied across all of an agent's strains.
...
- **Not yet modeled:** an explicit DST-indeterminate outcome, partial (per-drug) TPT efficacy (sterilization is all-or-nothing per strain), and loss-to-follow-up as a separate treatment outcome.
