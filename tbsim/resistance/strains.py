...
class StrainSpec:
    ...
    Args:
        ...
        acquisition  (dict):  Optional per-drug random acquisition probabilities
    ...
    def __init__(self, uid, resistance, fitness=1.0, init_prev=0.0,
        ...
        if not isinstance(resistance, dict) or not resistance:
            raise ValueError(
                f'StrainSpec resistance must be a non-empty dict; got {resistance!r}'
            )
    ...
    def is_resistant_to(self, drug):
        """True if this strain is resistant to *drug*."""
