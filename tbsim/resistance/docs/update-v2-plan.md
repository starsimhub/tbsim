...
- `mutate_one_susceptible` ([strains.py:155-195](../strains.py)) currently returns only the new mask, discarding the `j→k` mapping. Extend it (or its callers) to also return the chosen source strain per agent so treatment `acquire` ([treatments.py:130-154](../treatments.py)) and TPT `_acquire` ([tpt.py:106-129](../tpt.py)) can transfer counts. The bottleneck path (`_bottleneck`) still legitimately uses `_sync_counts_to_mask` (survivor keeps its count, dropped strains → 0) and should be left as-is.
...
**Decision — D-TR1 — resolved (backcompat n/a).** Drop `acq_select` from `TxR` and switch `TxR.acquire` to the per-strain loop. **Open sub-question (modeling, not backcompat):** whether TPT `_acquire` should likewise move from single-pick to per-strain (symmetry with de-novo suggests it eventually should, but the notes only name treatment-acquired resistance). Recommend keeping `mutate_one_susceptible` in `strains.py` for now since TPT still uses it, and flagging the TPT-symmetry change for a follow-up decision rather than doing it in this pass unless confirmed.
...
- **Shared helpers.** Items 2/3/TR-1 all need a robust per-agent additive count-transfer primitive on `TBResistant`; build it once and route de-novo, treatment, and TPT acquisition through it. Item TR-5 wants the per-strain regimen-susceptible sterilization currently inside `TPTRx` factored onto `TBResistant` for reuse. [FEEDBACK: yes, and look for other simplification/refactoring opportunities, e.g. duplicated validation and other code between TB and TBResistant.]
