    ...
    def _apply_infection(self):
        ...
        if delta > 0:
            ...
            if delta > len(eligible):
                ss.warn(f"Not enough eligible people to infect. Expected: {delta}, Available: {len(eligible)}")
                return
            ...
            if -delta > len(acute_uids):
                ss.warn(f"Not enough acute cases to revert. Expected: {-delta}, Available: {len(acute_uids)}")
                return
    ...
    def _apply_art(self):
        ...
        if delta > 0:
            ...
            if delta > len(candidates):
                # raie a warning
                ss.warn(f"Not enough candidates for ART. Expected: {delta}, Available: {len(candidates)}")
                return
            ...
            if -delta > len(current_on_art):
                ss.warn(f"Not enough people on ART to remove. Expected: {-delta}, Available: {len(current_on_art)}")
                return
