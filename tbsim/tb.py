...
class TB(BaseTB):
    ...
    For multi-strain drug resistance, use
    :class:`~tbsim.resistance.multistrain_tb.MultiStrainTB` from ``tbsim.resistance``
    instead of passing ``strains=`` to this class.
...
def get_tb(sim, which=None):
    """Return the TB disease module from a simulation.

    Args:
        sim (Sim): Simulation to search.
        which (type, optional): Class to match (e.g. ``TB`` or
            :class:`~tbsim.resistance.multistrain_tb.MultiStrainTB``). If None,
            returns the first :class:`BaseTB` subclass found.
    """
    if which is None:
        which = BaseTB
