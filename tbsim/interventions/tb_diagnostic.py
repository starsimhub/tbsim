    ...
    def step(self):
        ...
        if len(tb_cases) > 0:
            try:
                ...
                raise AssertionError(f"TBDiagnostic failed to apply sensitivity test to TB cases: {e}. Sensitivity parameter: {self.pars.sensitivity}. Please check that sensitivity is a valid probability between 0.0 and 1.0.")
        ...
        if len(non_tb_cases) > 0:
            try:
                ...
                raise AssertionError(f"TBDiagnostic failed to apply specificity test to non-TB cases: {e}. Specificity parameter: {self.pars.specificity}. Please check that specificity is a valid probability between 0.0 and 1.0.")
        
        # Error handling: Check test result validity
        assert len(test_positive) == len(selected), f"TBDiagnostic test result length mismatch. Selected people: {len(selected)}, Test results: {len(test_positive)}. This indicates a problem with test application logic."
        assert test_positive.dtype == bool, f"TBDiagnostic test results must be boolean array, got {test_positive.dtype}. This indicates a problem with test application logic."
