...
Evidence base used for parameter defaults:
  - WHO consolidated guidelines on TB preventive treatment, 2nd ed. (2024)
    https://www.ncbi.nlm.nih.gov/books/NBK607293/
  - CDC/NTCA guidelines for treatment of LTBI (2020)
    https://www.cdc.gov/mmwr/volumes/69/rr/rr6901a1.htm
  - Ross et al. Lancet HIV 2021 (IPT+ART vs ART, N=2611): HR=0.68 (95%CI 0.49–0.95)
    https://pmc.ncbi.nlm.nih.gov/articles/PMC7875572/
  - Yothasan et al. Sci Rep 2023 (NMA, 15 RCTs, N=33,262)
    https://www.nature.com/articles/s41598-023-43310-8
  - Horton et al. PNAS 2023 (undulation paradigm, 49.5% undulate)
    https://pmc.ncbi.nlm.nih.gov/articles/PMC10666121/

Design changes from tpt.py:
  1.  Regimen catalog: WHO/CDC-recognized regimens with category-specific defaults
      (RegimenCategory, TPTRegimen, REGIMENS).
  2.  p_complete per regimen: agents who do not complete receive no protection.
  3.  activation_modifier calibrated to HR~0.68 (Ross et al.); clearance_modifier
      and death_modifier removed — no peer-reviewed evidence supports them.
  4.  Active TB exclusion covers NON_INFECTIOUS, ASYMPTOMATIC, SYMPTOMATIC, TREATMENT,
      not just TREATMENT (WHO: "active TB must be excluded before initiating TPT").
  5.  n_newly_initiated counted from administer() return value, not before it.
  6.  Protection expiry clears the offered flag, enabling re-offer after re-exposure
      (consistent with 49.5% undulation rate in Horton et al.).
  7.  TPTHousehold caches the household network and builds a UID index at init.
  8.  TPTHousehold uses two separate coverage parameters: household-visit probability
      and per-contact acceptance probability.
"""
