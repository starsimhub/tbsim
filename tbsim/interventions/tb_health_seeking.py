...
class HealthSeekingBehavior(ss.Intervention):
    ...
    def __init__(self, pars=None, **kwargs):
        ...
        self.define_states(
            ss.IntArr(
                "n_care_sought", default=0
            ),  # times sought in current episode; resets when leaving eligible states
    ...
    def init_post(self):
        ...
        if self.pars.custom_states is not None:
            ...
            if not np.isin(self._states, self.tbsl.care_seeking_eligible()).all():
                raise ValueError(
                    "Custom states must be a subset of the eligible states."
                )
        else:
        ...
        if self.pars.start is None:
            self.pars.start = self.sim.t.start
        if self.pars.stop is None:
            self.pars.stop = self.sim.t.stop
        return
    ...
    def step(self):
        ...
        if (
            self.pars.care_retry_steps is not None
            and int(self.pars.care_retry_steps) > 0
        ):
            can_retry = (self.ti - self.ti_last_sought) >= int(
                self.pars.care_retry_steps
            )
        ...
        rate = (
            self.pars.initial_care_seeking_rate
        )  # do we need to reset this each time step?
    ...
    def update_results(self):
        ...
        if t < self.pars.start.date() or t > self.pars.stop.date():
            return
        ppl = self.sim.people
        self.results["n_sought_care"][self.ti] = np.count_nonzero(
            self.n_care_sought > 0
        )
        self.results["n_ever_sought_care"][self.ti] = np.count_nonzero(
            self.n_care_sought_total > 0
        )
