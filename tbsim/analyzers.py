...
class HarlemAnalyzer(ss.Analyzer):
    ...
    def finalize(self):
        super().finalize()
        self.df = pd.DataFrame(self.data, columns = ['year', 'arm', 'n_people', 'new_infections', 'new_active_infections', 'n_infected', 'n_died', 'n_latent_slow', 'n_micro_deficient', 'rel_LS_mean'])
