...
def update_version(pkg=tbsim):
    sc.heading('Updating docs version number...')
    filename = '_variables.yml'
    data = dict(version=tbsim.__version__, versiondate=tbsim.__versiondate__)
...
def customize_aliases(mod_name='tbsim', json_path='objects.json'):
    """
...
def build_objects_inv(json_path='objects.json', inv_path='objects.inv'):
    ...
    so other projects can resolve TBsim references via intersphinx.
    """
    import sphobjinv as soi
    sc.heading('Building Sphinx objects.inv ...')
    data = sc.loadjson(json_path)
    inv = soi.Inventory()
    inv.project = data.get('project', 'tbsim')
    inv.version = str(data.get('version', tbsim.__version__))
