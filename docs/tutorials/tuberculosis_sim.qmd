...
```python
tbsim.plot({'single simulation': sim.results.flatten(), 'tb only results': tbsim.get_tb(sim).results.flatten()})
```
...
tbsim.plot({'ALL SIM metrics': sim.results.flatten(),'TB ONLY metrics': tbsim.get_tb(sim).results.flatten()}, n_cols=4, row_height=2 )
```
...
fig = plt.figure()
tb_results = tbsim.get_tb(sim).results
