...
class TwoStrainODE(sc.prettyobj):
    ...
    Args:
        kwargs: parameter overrides applied on top of ``two_strain_defaults()``.

    Usage::

        ode = TwoStrainODE(beta=16.45)
        df = ode.run(SY_A=1e3, SY_B=1e1)   # returns a DataFrame of compartments + observables
    """
    ...
    def init_conditions(self, **seeds):
        """Seed counts by state name; SUS absorbs the balance so the 22 compartments sum to N."""
    ...
    def derivs(self, y, t):
        ...
        # Progression bottleneck into ASYMPTOMATIC from AB sources
    ...
    def run(self, start_time=1500, end_time=2020, by=1, **seeds):
        """Integrate and return a DataFrame of compartments + key observables (one row per year)."""
        if not seeds:
    ...
    def collapse(self):
        """Strain-summed view (single-strain state names) for compartment-for-compartment comparison."""
        d = self.df
        return sc.dataframe(
